...
class Settings(PluginSettings):
    settings = {
        ...
        "rename_file":                  False,
        "search_patten":                "name_S{$season}_E{$episode}",
        "rename_template":              "S{$season}_E{$episode}",
    }
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.form_settings = {
            ...
            "search_patten":                self.__show_when(
                "rename_file",
                "Search patten : use {$key} to mark variable"
            ),
            "rename_template":              self.__show_when(
                "rename_file",
                "New name template : use {$key} to inset variable"
            ),
        }
    ...
    def __show_when(self, key, label):
        values = {
            "label": label
        }
...
def transform_basename(basename: str, search: str, template: str):
    """
    Match `basename` against `search` (which contains placeholders like {$s}, {$e}, {$name}).
    If match succeeds, return the `template` with placeholders replaced by captured values.
    If match fails, return None.

    Example:
        transform_basename("unformat_name_03_32", "unformat_name_{$s}_{$e}", "format_name_S{$s}E{$e}")
        -> "format_name_S03E32"
    """

    import re
    PLACEHOLDER_RE = re.compile(r'\{\$([A-Za-z0-9_]+)\}')
    # split search into literal and placeholder tokens
    parts = re.split(r'(\{\$[A-Za-z0-9_]+\})', search)
    # collect placeholder names in order
    placeholders = [m.group(1) for m in PLACEHOLDER_RE.finditer(search)]
    if not placeholders and '{' in search:
        # malformed placeholder usage: no valid names found
        raise ValueError("No valid placeholders found in search pattern")

    # build regex pattern from parts; use non-greedy for all placeholders except the last
    pattern_parts = []
    last_idx = len(placeholders) - 1
    ph_seen = 0
    for part in parts:
        if not part:
            continue
        ...
        if m:
            name = m.group(1)
            if ph_seen == last_idx:
                grp = rf'(?P<{name}>.+)'   # last placeholder: greedy
            else:
                grp = rf'(?P<{name}>.+?)'  # other placeholders: non-greedy
            pattern_parts.append(grp)
            ph_seen += 1
        else:
    ...
    # replace placeholders in template with matched group values (missing -> empty string)
    def repl(match_obj):
        key = match_obj.group(1)
        return m.groupdict().get(key, '')

    result = PLACEHOLDER_RE.sub(repl, template)
    return result
...
def get_file_out(settings, original_source_path, file_out, library_id=None):
    ...
    if settings.get_setting('rename_file'):
        original_name, ext = os.path.splitext(file_out_basename)
        file_out_basename = transform_basename(
            basename = original_name,
            search = settings.get_setting("search_patten"),
            template = settings.get_setting("rename_template")
        ) + ext
